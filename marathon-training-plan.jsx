...
const FUELING = {
  ...
  sections: [
    {
      name: "During Long Runs (>60 min)",
      items: [
        "30–60g carbs per hour after the first 45 minutes",
        "Options: energy gels (GU, Maurten, SiS), chews (Clif Bloks), or even gummy bears",
        "Wash down each gel with 4–6 oz of water — not sports drink (double sugar = GI distress)",
        "Practice every long run. Never try new fuel on race day.",
      ],
    },
    {
      name: "Pre-Run (60–90 min before)",
      items: [
        "200–300 calories of easy-to-digest carbs: toast w/ honey, banana, oatmeal, rice cake",
        "8–12 oz water. Small amount of coffee is fine if you're used to it.",
    ...
    },
    {
      ...
      items: [
        "Carbs are your fuel: 3–5g/kg bodyweight on easy days, 5–7g/kg on hard/long days",
        "Protein: 1.4–1.7g/kg for recovery and muscle maintenance",
        "Don't skimp on sodium — you lose 500–1500mg per hour of running in heat",
        "Hydrate to thirst. Aim for pale yellow urine as a general gauge.",
      ],
    },
    {
      name: "Race Week Carb Load",
      items: [
        "3 days out: increase carbs to 7–10g/kg bodyweight per day",
        "This is not just 'eat pasta' — it's rice, bread, potatoes, juice, pretzels all day",
        "Reduce fiber and fat to make room for carbs without overeating total volume",
        "Pre-race dinner: familiar, carb-heavy, low-fiber. White rice + chicken is a classic.",
      ],
    },
  ],
};
    ...
    <div style={{ fontFamily: "'Source Sans 3', 'Source Sans Pro', sans-serif", maxWidth: 720, margin: "0 auto", padding: "16px", color: "#1a1a1a" }}>
            ...
            {WEEKS.map((w, i) => (
              <button
                ...
                style={{
                  ...
                  padding: "8px 4px",
                  fontSize: 11,
                  fontWeight: selectedWeek === i ? 700 : 500,
                  fontFamily: "inherit",
                  background: selectedWeek === i ? "#1a1a1a" : "#f5f5f5",
                  color: selectedWeek === i ? "#fff" : "#666",
                  border: "none",
                  borderRadius: 6,
                  cursor: "pointer",
                  textAlign: "center",
              ...
              >
                <div>Wk {w.week}</div>
                <div style={{ fontSize: 10, opacity: 0.7 }}>{w.totalMiles} mi</div>
              </button>
            ))}
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
            <div>
              ...
              <span style={{
                ...
                marginLeft: 10,
                padding: "3px 8px",
                background: PHASE_COLORS[week.phase] + "18",
                borderRadius: 4,
              }}>
                {week.phase}
              </span>
            </div>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#666" }}>{week.totalMiles} miles</div>
          </div>
          ...
          <div style={{ display: "flex", gap: 3, alignItems: "flex-end", height: 40, marginBottom: 16, padding: "0 2px" }}>
            {WEEKS.map((w, i) => (
              <div
                ...
                style={{
                  flex: 1,
                  height: `${(w.totalMiles / 44) * 100}%`,
                  background: selectedWeek === i ? "#1a1a1a" : "#e0e0e0",
                  borderRadius: 2,
                  transition: "all 0.2s",
                  minHeight: 4,
                }}
            ...
            {week.days.map((d, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: "#fff",
                    border: `1px solid ${isExpanded ? colors.border : "#e8e8e8"}`,
                    borderRadius: 8,
                    padding: "12px 14px",
                    cursor: "pointer",
                    transition: "all 0.15s",
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 36, fontSize: 12, fontWeight: 600, color: "#999" }}>{d.day}</div>
                    <span style={{
                      fontSize: 10,
                      fontWeight: 700,
                      padding: "2px 7px",
                    ...
                    <div style={{ fontSize: 13, fontWeight: 500, flex: 1 }}>{d.desc}</div>
                    {d.miles > 0 && (
                      <div style={{ fontSize: 12, fontWeight: 600, color: "#999", flexShrink: 0 }}>{d.miles} mi</div>
                    )}
                  </div>
                  {isExpanded && (
                    <div style={{
      ...
      {activeTab === "fuel" && (
        <div>
          ...
          {FUELING.sections.map((section, i) => (
            <div key={i} style={{ marginBottom: 20 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 8, color: "#333" }}>{section.name}</h3>
          ...
          <div style={{
            ...
            <strong>Your 11.8-mile lesson:</strong> You bonked because you ran 90+ minutes on just a glass of water. With proper fueling, that same effort gets you to 13.1 and beyond. Every long run from here on is a nutrition rehearsal.
          </div>
        </div>
      )}
      ...
      {activeTab === "race" && (
        <div>
          ...
          <div style={{ fontSize: 13, color: "#555", marginBottom: 16, lineHeight: 1.6 }}>
            The #1 mistake in a marathon is going out too fast. The first miles feel amazing and that's the trap. Start slower than you think you should. The race begins at mile 20.
          </div>
          {RACE_STRATEGY.paces.map((p, i) => (
            <div key={i} style={{
              display: "flex",
              gap: 14,
              padding: "12px 0",
              borderBottom: i < RACE_STRATEGY.paces.length - 1 ? "1px solid #f0f0f0" : "none",
              alignItems: "flex-start",
            }}>
              <div style={{ width: 90, flexShrink: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 700 }}>{p.segment}</div>
                <div style={{ fontSize: 16, fontWeight: 300, color: "#1565C0", marginTop: 2 }}>{p.pace}</div>
              </div>
          ...
          <div style={{
            ...
            <strong>Fuel stations:</strong> {RACE_STRATEGY.fuel}
          </div>
          <div style={{
            marginTop: 12,
            padding: 14,
            ...
            <strong>Target finish:</strong> ~3:55:00. That gives you a 5-minute buffer under 4:00. If you execute the pacing plan and fuel properly, you'll have room to push the last 10K.
          </div>
        </div>
      )}
    </div>
